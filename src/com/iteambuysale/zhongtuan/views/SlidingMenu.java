...
//	private View mDetailView;
	...
	public boolean hasClickLeft = false;
	...
	public void setLeftView(View view) {
		...
		mMenuView = view;
		view.measure(0, 0);
	   //Toast.makeText(mContext, "mMenuview width:"+view.getMeasuredWidth(), 1).show();
	}
	...
	public void setCanSlidingleft(boolean left) {
		canSlideLeft = left;
		//canSlideRight = right;
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			if (mIsBeingDragged) {
				...
				float xVelocity = velocityTracker.getXVelocity();// 滑动的ꀥڿ				
				...
				if (oldScrollX <= 0 && canSlideLeft) {// left view
					if (xVelocity > VELOCITY) {
						...
						if (hasClickLeft) {
							hasClickLeft = false;
							setCanSlidingleft(tCanSlideLeft);
						}
	...
	public void showLeftView() {
		...
		int oldScrollX = mSlidingView.getScrollX();
		//Toast.makeText(mContext, "oldScrollX:"+oldScrollX, 1).show();
		if (oldScrollX == 0) {
			mMenuView.setVisibility(View.VISIBLE);
		/*	if (mDetailView!=null){
				mDetailView.setVisibility(View.INVISIBLE);
