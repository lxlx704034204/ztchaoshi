...
public class DBUtilities {
    ...
    public static Cursor getallShop() {
        SQLiteDatabase db = mInstance.getRDB();
        return db.rawQuery("select * from Shop_Msg order by _xh desc,_id desc ", null);
    }
    ...
    public static Cursor getUnpayOrder() {
        SQLiteDatabase db = mInstance.getRDB();
        return db.query("ORDER_TM_LIST", null, "_ordzt = ?",
     ...
     * @return
     */
    public static Cursor getOrderByoderNumber(String orderno) {
        SQLiteDatabase db = mInstance.getRDB();
        return db.rawQuery("select * from ORDER_TM_LIST where _ordno =? order by _id desc", new String[]{orderno});
    }

    /**
     * 从ORDER_DETAILS_TM表里面获取 订单号为orderno的子订单号
     *
     * @param orderno
     * @return
     */
     ...
     * 根据大类获取小类的id
     */
    public static Cursor getsmallcategory(String cup) {
        SQLiteDatabase db = mInstance.getRDB();
        //System.out.println("select * from TEMAI_CATAGORY where _cup="+cup);
        return db.rawQuery("select * from TEMAI_CATAGORY where _cup= ? ", new String[]{cup});
    }
    /*
    *
    *  更具小订单获取次产品的物流单号
    * */
    public static String getLogistic(String ordnox) {
        SQLiteDatabase db = mInstance.getRDB();
        Cursor cursor = db.rawQuery("select _logid from ORDER_DETAILS_TM where _ordnox=?", new String[]{ordnox});
        System.out.println("cursor length:"+cursor.getCount());
        if (cursor.moveToFirst()){

            return cursor.getString(cursor.getColumnIndex("_logid"));
        }else{
            return null;
